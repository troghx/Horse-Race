...
async function ensureTickerConfig(tickerStore, seedConfig) {
  ...
  if (isValidTickerConfig(current)) {
    if (isValidTickerConfig(seedConfig) && getConfigVersion(seedConfig) > getConfigVersion(current)) {
      const seeded = normalizeTickerConfig(seedConfig);
      try {
        await tickerStore.write(seeded);
      } catch {
        // Si Blobs falla, degradamos al seed para no tumbar la app publica.
      }
      return seeded;
    }
