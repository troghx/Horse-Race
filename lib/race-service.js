...
const parsedCacheTtl = Number(process.env.SHEETS_CACHE_TTL_MS || 120_000);
const CACHE_TTL_MS = Number.isFinite(parsedCacheTtl) && parsedCacheTtl > 0 ? parsedCacheTtl : 120_000;
export const SHEET_API_CACHE_CONTROL = "public, max-age=30, stale-while-revalidate=270";
...
export function createFileTeamStore(teamsFile) {
  return createFileJsonStore(teamsFile);
}

export function createFileJsonStore(filePath) {
  return {
    async read() {
      try {
...
export function jsonResponse(status, body, options = {}) {
...
function sanitizeTickerMessages(messages) {
...
function sanitizePrizeAwards(awards) {
  if (!isPlainObject(awards)) return {};

  const cleanAwards = {};

  for (const [slot, amount] of Object.entries(awards)) {
    if (!PRIZE_SLOTS.has(slot)) continue;
    const parsedAmount = Number(amount);
    if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) continue;
    cleanAwards[slot] = Math.round(parsedAmount);
  }

  return cleanAwards;
}

function isValidTickerConfig(config) {
  return isPlainObject(config) && Array.isArray(config.items);
}

function createDefaultTickerConfig() {
  return {
    version: 1,
    updatedAt: "",
    items: [...DEFAULT_TICKER_MESSAGES],
  };
}

function normalizeTickerConfig(config) {
  const base = isValidTickerConfig(config) ? config : createDefaultTickerConfig();
  const items = sanitizeTickerMessages(base.items);

  return {
    version: getConfigVersion(base) || 1,
    updatedAt: typeof base.updatedAt === "string" ? base.updatedAt : "",
    items: items.length ? items : [...DEFAULT_TICKER_MESSAGES],
  };
}

function isValidPrizeModeConfig(config) {
  return isPlainObject(config);
}

function createDefaultPrizeModeConfig() {
  return {
    version: 1,
    updatedAt: "",
    active: false,
...
function normalizePrizeModeConfig(config) {
  const base = isValidPrizeModeConfig(config) ? config : createDefaultPrizeModeConfig();

  return {
    version: getConfigVersion(base) || 1,
    updatedAt: typeof base.updatedAt === "string" ? base.updatedAt : "",
    active: Boolean(base.active),
    awards: sanitizePrizeAwards(base.awards),
  };
}

async function ensureTickerConfig(tickerStore, seedConfig) {
  let current = null;
  try {
    current = await tickerStore.read();
  } catch {
    current = null;
  }

  if (isValidTickerConfig(current)) {
    return normalizeTickerConfig(current);
  }

  const fallback = isValidTickerConfig(seedConfig)
    ? normalizeTickerConfig(seedConfig)
    : createDefaultTickerConfig();

  try {
    await tickerStore.write(fallback);
  } catch {
    // Mantiene la app operativa aunque el store remoto no acepte la escritura.
  }

  return fallback;
}

async function ensurePrizeModeConfig(prizeStore, seedConfig) {
  let current = null;
  try {
    current = await prizeStore.read();
  } catch {
    current = null;
  }

  if (isValidPrizeModeConfig(current)) {
    return normalizePrizeModeConfig(current);
  }

  const fallback = isValidPrizeModeConfig(seedConfig)
    ? normalizePrizeModeConfig(seedConfig)
    : createDefaultPrizeModeConfig();

  try {
    await prizeStore.write(fallback);
  } catch {
    // Mantiene la app operativa aunque el store remoto no acepte la escritura.
  }
...
export async function createTickerPayload({ tickerStore, seedConfig }) {
  return ensureTickerConfig(tickerStore, seedConfig);
}

export async function saveTickerMessages({ items, tickerStore, seedConfig, baseVersion }) {
  const current = await ensureTickerConfig(tickerStore, seedConfig);
  const cleanItems = sanitizeTickerMessages(items);

  if (!cleanItems.length) {
    throw createHttpError("Agrega al menos una leyenda para el ticker", 400);
  }

  const requestedVersion = Number(baseVersion);
  const hasBaseVersion = Number.isFinite(requestedVersion) && requestedVersion > 0;
  if (hasBaseVersion && requestedVersion !== current.version) {
    throw createHttpError(
      "El ticker fue actualizado desde otro navegador. Recarga las leyendas e intenta de nuevo.",
      409,
      current,
    );
  }
  ...
  await tickerStore.write(nextConfig);
  return nextConfig;
}

export async function createPrizeModePayload({ prizeStore, seedConfig }) {
  return ensurePrizeModeConfig(prizeStore, seedConfig);
}

export async function savePrizeModeConfig({ prizeStore, seedConfig, active, awards, baseVersion }) {
  const current = await ensurePrizeModeConfig(prizeStore, seedConfig);
  const requestedVersion = Number(baseVersion);
  const hasBaseVersion = Number.isFinite(requestedVersion) && requestedVersion > 0;

  if (hasBaseVersion && requestedVersion !== current.version) {
    throw createHttpError(
      "El prize mode cambio desde otro navegador. Recarga la vista e intenta de nuevo.",
      409,
      current,
    );
  }
  ...
  const nextConfig = {
    ...
    awards: awards === undefined ? current.awards : sanitizePrizeAwards(awards),
  };

  await prizeStore.write(nextConfig);
  return nextConfig;
}
