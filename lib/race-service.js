...
function buildRace(entries, agentMeta, teamConfig, period, anchorDate) {
  ...
  const ranking = [...counts.entries()]
    .map(([agent, count]) => {
      ...
      const team = teamConfig.assignments[agent] || meta.team || "Sin equipo";
    ...
    .sort((a, b) => (b.count !== a.count ? b.count - a.count : a.agent.localeCompare(b.agent, "es")))
    .map((item, index, list) => {
...
export async function createRacePayload({ period, anchor, refresh, teamStore, sheetUrl, seedConfig }) {
  ...
  const race = buildRace(data.entries, data.agentMeta, teamConfig, normalizePeriod(period), anchorDate);
...
export async function createTeamsPayload({ teamStore, sheetUrl, seedConfig }) {
  ...
  for (const [agent, meta] of data.agentMeta) {
    assignments[agent] = config.assignments[agent] || meta.team || "Sin equipo";
  }
