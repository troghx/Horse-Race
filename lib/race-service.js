...
function computeInboundLevels(entries, rosterNames, anchorDate) {
  ...
  for (const entry of entries) {
    if (!entry.agent || !entry.campaign) continue;
    if (entry.date < monthStart || entry.date > monthEnd) continue;

    const key = normalizePersonKey(entry.agent);
    if (!key) continue;

    if (!agentCampaigns.has(key)) {
      agentCampaigns.set(key, { displayName: entry.agent, ecomfy: 0, radio: 0, atmedia: 0 });
    }

    const record = agentCampaigns.get(key);
    if (!record.displayName) record.displayName = entry.agent;
    if (record[entry.campaign] !== undefined) {
      record[entry.campaign] += 1;
    }
  }

  const agents = [];
  for (const [, record] of agentCampaigns) {
    agents.push({ agent: record.displayName, ecomfy: record.ecomfy, radio: record.radio, atmedia: record.atmedia });
  }
...
function computeAttendanceCompetition(entries, workforceData, start, end, agentMeta = new Map(), teamConfig = { teams: {}, assignments: {} }) {
  ...
  const rankedAgents = agents
    .sort((a, b) => {
      ...
      if (a.deals !== b.deals) return a.deals - b.deals;
...
export async function createRacePayload({ period, anchor, refresh, teamStore, sheetUrl, seedConfig }) {
  ...
  const race = buildRace(data.entries, data.agentMeta, teamConfig, normalizePeriod(period), anchorDate, workforceData.genderMap);
...
export async function createInboundPayload({ refresh, sheetUrl }) {
  if (refresh) cache.expiresAt = 0;
  ...
  const rosterNames = [...workforceData.genderMap.keys()]
    .map((key) => {
      // Try to find a display name from sales entries
      for (const entry of data.entries) {
        if (normalizePersonKey(entry.agent) === key) return entry.agent;
      }
      return key;
    });
